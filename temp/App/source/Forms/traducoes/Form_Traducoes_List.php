...
class Form_Traducoes_List extends PRESTO_Forms
{
    public function __construct()
    {
        #titulo
        $titulo = new PRESTO_Components_Title('titulo');
        $titulo->setText($this->translate("Traducoes"));
        ...
        $input_search->setPlaceHolder($this->translate("Localizar por chave"));
        $input_search->setLabel($this->translate("Localizar"));
        $input_search->setMethod('post');
        $input_search->setClass('form-control');
        $input_search->setTitle($this->translate('Digite a chave  a ser localizada.'));
        $input_search->setAction(APP_PATH . "/traducoes/");
        ...
        $pagination->setTotalRecords((new TraducoesTable())->getTotalRecords(['count']));
        ...
        $btnAdd->setText($this->translate('Adicionar Nova Traducao'));
        $btnAdd->setTitle('Adiciona uma nova traducao');
